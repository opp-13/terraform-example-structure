...
두 role의 신뢰 정책은 GitHub OIDC 토큰의 **`job_workflow_ref` claim**(어느 워크플로우 *파일*에서 나온 토큰인지 - `terraform-pr.yml`인지 `terraform-apply.yml`인지)으로 스코핑합니다. `sub` claim이 아닌 이유: `plan`/`apply` job은 tfvars 값을 읽으려고 `environment: <env>`(순수 설정 저장용, protection rule 없음)를 선언하는데, **GitHub는 job에 environment가 선언되어 있으면 이름과 무관하게 `sub`를 항상 `repo:...:environment:<name>` 형태로 만듭니다** — 그래서 `sub`로는 "PR에서 도는 plan인지 main push apply인지"를 구분할 방법이 없습니다. `job_workflow_ref`는 environment 선언 여부와 무관하게 항상 어느 워크플로우 파일이 실행됐는지를 담고 있어서, 그걸로 plan/apply를 정확히 나눕니다.
...
`<env>-plan`/`<env>-apply`처럼 role 종류별로 나눈 Environment는 필요 없습니다 — OIDC 신뢰 조건이 이제 GitHub Environment 이름이 아니라 `job_workflow_ref`(워크플로우 파일 기준) claim으로 스코핑되기 때문입니다.
